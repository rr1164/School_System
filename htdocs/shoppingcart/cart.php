...
if (isset($_POST['paypal']) && $products_in_cart && !empty($products_in_cart)) {
    ...
    $data = array(
        ...
        'upload'        => '1',
        'lc'			=> 'EN',
        'business' 		=> 'payments@http://testfff.epizy.com',
        'cancel_return'	=> 'http://testfff.epizy.com/shoppingcart/index.php?page=cart',
        'notify_url'	=> 'http://testfff.epizy.com/shoppingcart/index.php?page=cart&ipn_listener=paypal',
        'currency_code'	=> 'USD',
        'return'        => 'http://testfff.epizy.com/shoppingcart/index.php?page=placeorder'
    );
    // Add all the products that are in the shopping cart to the data array variable
    for ($i = 0; $i < count($products); $i++) {
        $data['item_number_' . ($i+1)] = $products[$i]['id'];
